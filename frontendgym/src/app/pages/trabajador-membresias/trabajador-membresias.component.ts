...
@Component({
  selector: 'app-trabajador-membresias',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, TrabajadorNavComponent],
  templateUrl: './trabajador-membresias.component.html',
  styleUrls: ['./trabajador-membresias.component.scss']
})
export class TrabajadorMembresiasComponent implements OnInit {
  membresias: any[] = [];
  ...
  cargar() {
    ...
    this.ws.listarSuscripciones(params).subscribe({
      next: (list) => { this.membresias = list || []; this.cargando = false; },
