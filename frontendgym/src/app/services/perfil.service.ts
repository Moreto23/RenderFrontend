...
export class PerfilService {
  ...
  editar(usuarioId: number, dto: any) {
    return this.http.patch<any>(`${this.base}`, dto, { params: toHttpParams({ usuarioId }) });
  }

  password(usuarioId: number, nueva: string) {
    return this.http.patch<any>(`${this.base}/password`, { nueva }, { params: toHttpParams({ usuarioId }) });
  }

  pedidos(usuarioId: number) {
    return this.http.get<any[]>(`${this.base}/pedidos`, { params: toHttpParams({ usuarioId }) });
  }

  planes(usuarioId: number) {
    return this.http.get<any[]>(`${this.base}/planes`, { params: toHttpParams({ usuarioId }) });
  }
