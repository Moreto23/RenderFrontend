...
export class CarritoService {
  ...
  private base = `${environment.apiBaseUrl}/api/carrito`;
  ...
  agregar(usuarioId: number, productoId: number, cantidad = 1) {
    return this.http.post<any>(`${this.base}`, null, {
      params: toHttpParams({ usuarioId, productoId, cantidad })
    });
  }

  actualizar(itemId: number, cantidad: number) {
    return this.http.patch<any>(`${this.base}/${itemId}`, null, {
      params: toHttpParams({ cantidad })
    });
  }
  ...
  agregarConToken(productoId: number, cantidad = 1) {
    // El backend saca el usuario del JWT
    return this.http.post<any>(`${this.base}`, null, {
      params: toHttpParams({ productoId, cantidad })
    });
  }
