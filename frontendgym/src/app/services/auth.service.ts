...
export class AuthService {
  ...
  // Antes: private readonly BASE = '/api/auth';
  ...
  registerConfirm(email: string, code: string) {
    return this.http.post<{status:string; message:string}>(`${this.BASE}/register-confirm`, null, {
      params: { email, code }
    });
  }
  ...
  recoverInit(email: string): Observable<{status:string; message:string}> {
    return this.http.post<{status:string; message:string}>(`${this.BASE}/recover-init`, null, {
      params: { email }
    });
  }
